    ...
    describe("transferFrom", function () {
        describe("failure", function () {
            it("should fail to transfer soul-bound token", async function () {
                await healthRecordNFT.connect(cipherHealth).mint(testAddress.address);
                await expect(healthRecordNFT.connect(testAddress).transferFrom(testAddress.address, deployer.address, 0)).to.be.revertedWithCustomError(
                    healthRecordNFT,
                    "HealthRecordNFT__NotAuthorizedForTransfer",
                );
            });
        });
    });
